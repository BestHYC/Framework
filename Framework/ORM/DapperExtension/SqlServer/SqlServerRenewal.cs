...
{
    public class SqlServerRenewal<T> : Renewal<T> where T : IEntity, new()
    {
        ...
        {
            HashSet<String> defaults = EntityTableMapper.DefaultColumn(typeof(T));
            ...
            {
                //去掉主键
                if (defaults.Contains(cols[i])) continue;
                if(colstr.Length != 0)
                {
                    colstr.Append(",");
                    valstr.Append(",");
